...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("addSchedule").onclick = () => container.appendChild(createScheduleBlock());
  ...
  div.innerHTML = `
    ...
    <select class="sch-day">
      ${["Thứ 2","Thứ 3","Thứ 4","Thứ 5","Thứ 6","Thứ 7","Chủ nhật"]
        .map(d => `<option ${s.dayOfWeek === d ? "selected" : ""}>${d}</option>`).join("")}
    </select>
    ...
    <button type="button" class="remove-btn btn-danger">❌ Xóa</button>
  `;
  div.querySelector(".remove-btn").onclick = () => div.remove();
  return div;
}

// ─── Load dữ liệu môn hiện tại ───────────────────────────
async function loadCourse() {
  try {
    ...
    if (data.schedule?.length > 0) {
      data.schedule.forEach(s => container.appendChild(createScheduleBlock(s)));
    } else {
...
function validateSchedules() {
  ...
  if (!valid) alert("⚠️ Giờ kết thúc phải sau giờ bắt đầu!");
  return valid;
}
